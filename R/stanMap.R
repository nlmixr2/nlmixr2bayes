...
.stanMap <- function(ui) {
  ...
  .et <- .iniDf[!is.na(.iniDf$neta1) & .iniDf$neta1 == .iniDf$neta2, , drop = FALSE]
  .et <- .et[order(.et$neta1), , drop = FALSE]
  .eta <- data.frame(name = .et$name, neta = .et$neta1, est = .et$est, fix = .et$fix, stringsAsFactors = FALSE)
  ...
  for (.b in .blocks) {
    if (!identical(.b$idx, seq(.b$start, .b$end))) {
      ...
      stop(
        "omega block over ",
        paste(.b$members, collapse = ", "),
        " is not contiguous in eta order",
        call. = FALSE
      )
      # nocov end
    }
...
.stanAssertThetaGradCover <- function(map, thetaSensIdx) {
  ...
  .uncovered <- setdiff(.free, Reduce(union, list(.muCovered, .covCovered, .sensCovered, .mixCovered)))
  if (length(.uncovered) > 0L) {
    stop(
      "no gradient source (mu-reference or theta sensitivity) for ",
...
.stanMuRefCovValues <- function(map, dataSav) {
  ...
  for (.j in seq_len(nrow(.mrc))) {
    ...
    if (!.cn %in% names(dataSav)) {
      stop("mu-referenced covariate '", .cn, "' is not a data column", call. = FALSE) # nocov
    }
    .cv <- dataSav[[.cn]]
    for (.i in seq_len(.nid)) {
      .u <- unique(.cv[.id == .i & !is.na(.cv)])
      if (length(.u) == 0L) {
        stop("mu-referenced covariate '", .cn, "' has no value for ", "subject ", .i, call. = FALSE)
      }
      if (length(.u) > 1L) {
        .tv[.j] <- TRUE
        break
