...
.stanOmegaBlockSpec <- function(block, omPri, ctl) {
  ...
  if (any(block$fix)) {
    if (block$k > 1L) {
      stop(
        "the omega block over ",
        paste(block$members, collapse = ", "),
        " contains fixed eta(s) with modeled correlations; ",
        "est=\"stan\" supports fixed DIAGONAL etas only",
        call. = FALSE
      )
    }
    if (any(omPri$name %in% block$members)) {
      stop("eta '", .first, "' is fixed; a prior on a fixed eta is a ", "contradiction", call. = FALSE)
    }
    return(list(type = "fixed", default = FALSE, nu = NULL, scale = NULL, lkjEta = NULL))
  }
  ...
  if (nrow(.row) == 0L) {
    ...
    return(list(
      type = if (block$k == 1L) "sd" else "lkj",
      default = TRUE,
      nu = NULL,
      scale = NULL,
      lkjEta = ctl$lkjEta
  ...
  if (.lk$kind != "matrix") {
    stop(
      ...
      "values are not yet supported by est=\"stan\"",
      call. = FALSE
    )
  }
  if (.lk$stanName %in% c("inv_wishart", "wishart", "inv_wishart_cholesky", "wishart_cholesky")) {
...
.stanGenerate <- function(ui, map, pri, ctl) {
  ...
  # IOV magnitude thetas ARE occasion standard deviations (nlmixr2est's IOV
  # rewrite, see R/stanIov.R); one without a declared prior gets the same
  ...
  # up with
  .iovDefault <- setdiff(intersect(.stanIovMagnitude(ui$iniDf), .free$name), pri$pop$name)
  # flat thetas: allowed, but say so loudly
  .flat <- setdiff(.free$name, c(pri$pop$name, .iovDefault, unlist(strsplit(stats::na.omit(pri$pop$members), ","))))
  if (length(.flat) > 0L) {
    .notes <- c(.notes, paste0("flat (improper) priors on: ", paste(.flat, collapse = ", ")))
  }
  ...
  if (map$nMix > 2L) {
    stop(
      "est=\"stan\" supports 2-component mixtures for now (this model ",
      "has ",
      map$nMix,
      " components)",
      call. = FALSE
  ...
  for (.b in seq_along(map$blocks)) {
    ...
    if (.sp$type == "fixed") {
      ...
      .tpL <- c(.tpL, paste0("  matrix[1,1] L", .id, " = [[", .stanNum(sqrt(.blk$init[1, 1])), "]];"))
    } else if (.sp$type == "cov") {
      .decl <- c(.decl, paste0("  cov_matrix[", .k, "] omega", .id, ";"))
      .priorL <- c(
        .priorL,
        paste0("  omega", .id, " ~ ", .sp$stanName, "(", .stanNum(.sp$nu), ", ", .stanMatLit(.sp$scale), ");")
      )
      .tpL <- c(.tpL, paste0("  matrix[", .k, ",", .k, "] L", .id, " = cholesky_decompose(omega", .id, ");"))
    } else if (.sp$type == "sd") {
      .decl <- c(.decl, paste0("  real<lower=0> sd", .id, ";"))
      .sdPrior <- sprintf(ctl$diagOmegaSdPrior, .stanNum(2.5 * sqrt(.blk$init[1, 1])))
      .priorL <- c(.priorL, paste0("  sd", .id, " ~ ", .sdPrior, ";"))
      if (.sp$default) {
        .notes <- c(.notes, paste0("default prior sd", .id, " ~ ", .sdPrior, " on omega diagonal '", .blk$members, "'"))
      }
      ...
      .priorL <- c(.priorL, paste0("  Lcorr", .id, " ~ lkj_corr_cholesky(", .stanNum(.sp$lkjEta), ");"))
      for (.j in seq_len(.k)) {
        .sdPrior <- sprintf(ctl$diagOmegaSdPrior, .stanNum(2.5 * sqrt(.blk$init[.j, .j])))
      ...
      if (.sp$default) {
        .notes <- c(
          .notes,
          paste0(
            "default priors lkj_corr_cholesky(",
            .stanNum(.sp$lkjEta),
            ") + half-Cauchy SDs",
            " on omega block '",
      ...
      .tpL <- c(
        .tpL,
        paste0("  matrix[", .k, ",", .k, "] L", .id, " = diag_pre_multiply(sd", .id, ", Lcorr", .id, ");")
      )
    }
    if (identical(ctl$etaParam, "centered")) {
      ...
      .priorL <- c(
        ...
        paste0(
          ...
          ") etaP",
          .id,
          "[i] ~ multi_normal_cholesky(rep_vector(0, ",
          .k,
          "), L",
          .id,
          ");"
      ...
      .tpL <- c(.tpL, paste0("  eta[, ", .blk$start, ":", .blk$end, "] = etaP", .id, ";"))
    } else {
      .decl <- c(.decl, paste0("  matrix[", .rowsN, ", ", .k, "] z", .id, ";"))
      .priorL <- c(.priorL, paste0("  to_vector(z", .id, ") ~ std_normal();"))
      .tpL <- c(.tpL, paste0("  eta[, ", .blk$start, ":", .blk$end, "] = z", .id, " * L", .id, "';"))
    }
    .gqOmega <- c(
      .gqOmega,
      paste0("  omegaOut[", .blk$start, ":", .blk$end, ", ", .blk$start, ":", .blk$end, "] = L", .id, " * L", .id, "';")
    )
    if (.sp$type != "fixed") {
      .dispOm <- c(.dispOm, paste0("    matrix[", .k, ",", .k, "] Od", .id, " = L", .id, " * L", .id, "';"))
      for (.di in seq_len(.k)) {
        for (.dj in seq_len(.di)) {
          .dispIdx <- .dispIdx + 1L
          .dispOm <- c(.dispOm, paste0("    parDisp[", .dispIdx, "] = Od", .id, "[", .di, ",", .dj, "];"))
          .dispNames <- c(
            .dispNames,
            if (.di == .dj) {
              paste0("om.", .blk$members[.di])
            } else {
              paste0("cov.", .blk$members[.di], ".", .blk$members[.dj])
            }
          )
        }
      }
  ...
  for (.i in seq_len(nrow(map$theta))) {
    .r <- map$theta[.i, ]
    .thL <- c(.thL, paste0("  theta[", .i, "] = ", if (.r$fix) .stanNum(.r$est) else .r$par, ";"))
  }
  ...
  .iniDf <- ui$iniDf
  .tbsRows <- which(.iniDf$err %in% c("boxCox", "yeoJohnson") & !.iniDf$fix & !is.na(.iniDf$ntheta))
  .tbsDecl <- character(0)
  ...
  for (.i in .tbsRows) {
    ...
    .tbsDecl <- c(.tbsDecl, paste0("  real ", .dn, ";  // DV-transform Jacobian statistic"))
    .tbsTarget <- c(.tbsTarget, paste0("  target += (", map$theta$par[.p], " - 1) * ", .dn, ";"))
    .tbsJac[[length(.tbsJac) + 1L]] <-
      list(name = .dn, theta = .iniDf$name[.i], transform = .iniDf$err[.i], condition = .iniDf$condition[.i])
  }
  # ---- assemble -----------------------------------------------------------
  if (nrow(map$eta) == 0L) {
    ...
    if (any(map$theta$fix)) {
      stop(
        "population-only est=\"stan\" needs fixed thetas dissolved: ",
  ...
  .code <- paste(
    c(
      ...
      if (map$nMix > 1L) {
        ...
        c(
          paste0("  {"),
          paste0("    vector[", .rowsN, "] llCond = nlmixr2_cond_all2(eta, theta);"),
          paste0("    real sumLL = 0;"),
          paste0("    for (i in 1:N) {"),
          paste0("      sumLL += log_sum_exp(log(", .mixP, ") + llCond[i], log1p(-", .mixP, ") + llCond[N + i]);"),
          paste0("    }"),
          paste0("    target += sumLL;"),
          .dispTick,
          paste0("  }")
      ...
      "  vector[N] logLikSubj;",
      if (map$nMix > 1L) paste0("  matrix[N, ", map$nMix, "] mixProbOut;"),
      .gqOmega,
      if (map$nMix > 1L) {
        c(
          paste0("  {"),
          paste0("    vector[", .rowsN, "] llCond = nlmixr2_cond_all2(eta, theta);"),
          paste0("    for (i in 1:N) {"),
          paste0("      real lp1 = log(", .mixP, ") + llCond[i];"),
          paste0("      real lp2 = log1p(-", .mixP, ") + llCond[N + i];"),
...
.stanInit <- function(map, blockSpecs, nid, ctl) {
  ...
  lapply(seq_len(ctl$chains), function(.chain) {
    ...
    for (.i in seq_len(nrow(.free))) {
      ...
      if (is.finite(.r$lower)) {
        .v <- max(.v, .r$lower + 1e-4 * max(1, abs(.r$lower)))
      }
      if (is.finite(.r$upper)) {
        .v <- min(.v, .r$upper - 1e-4 * max(1, abs(.r$upper)))
      }
      .ini[[.r$par]] <- .v
    }
    for (.b in seq_along(blockSpecs)) {
      ...
      if (identical(ctl$etaParam, "centered")) {
        .ini[[paste0("etaP", .id)]] <- matrix(.jit * stats::rnorm(nid * .k), nid, .k)
      } else {
        .ini[[paste0("z", .id)]] <- matrix(.jit * stats::rnorm(nid * .k), nid, .k)
      }
    }
    .ini
  })
