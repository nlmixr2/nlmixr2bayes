...
test_that("the bridge agrees with Stan's own ODE solver", {
  ...
  ev <- rxode2::et(ev, ts)
  h <- rxsRegister(pkModel, events = ev, sens = names(truth), output = "center", atol = absTol, rtol = relTol)
  ...
  smBridge <- rxsStanModel(
    system.file("stan", "pk_1cmt_oral.stan", package = "nlmixr2bayes"),
    modelName = "xcheck_bridge"
  )
  fitBridge <- rstan::sampling(
    smBridge,
    chains = 0,
    data = list(nObs = nObs, handle = as.integer(unclass(h)), cpObs = cpObs)
  )

  smStan <- rstan::stan_model(
    system.file("stan", "pk_1cmt_oral_stanode.stan", package = "nlmixr2bayes"),
    model_name = "xcheck_stanode"
  )
  fitStan <- rstan::sampling(
    smStan,
    chains = 0,
    data = list(
      nObs = nObs,
  ...
  for (delta in list(c(0, 0, 0, 0), c(0.3, -0.2, 0.15, 0.4), c(-0.5, 0.4, -0.3, -0.2))) {
