...
test_that("cond_batch_theta refuses without the tier-2 state installed", {
  ...
  .Call(`_nlmixr2bayes_clearThetaBase`)
  expect_error(
    .Call(`_nlmixr2bayes_condBatchTheta`, as.double(c(1, 3, 0.5)), matrix(0, h$nid, h$neta)),
    "status -101"
  )
})
...
test_that("dosed models work; dose-handling theta gradients carry the jump", {
  ...
  expect_equal(got$nBad, 0L)
  .h <- 1e-5
  fd <- (.condBatch(eta + .h)$value -
    .condBatch(eta - .h)$value) /
    (2 * .h)
  ...
  # assignments
  expect_equal(.stanEventThetas(rxode2::rxode2(.lagMod)), "tlag")
  if (!.stanHasEventThetaSens()) {
...
test_that("mu-referenced covariate coefficient gradients FD-agree", {
  ...
  .cv <- .stanMuRefCovValues(.map, .env$dataSav)
  ...
  .cvTv <- .stanMuRefCovValues(.map, .envTv$dataSav)
  ...
  .Call(`_nlmixr2bayes_setThetaBase`, as.double(h3$initPar))
  .Call(`_nlmixr2bayes_setMuRef`, as.integer(.map$muRefIdx))
  got3 <- .bt(th, eta)
