...
test_that("IOV expands to fixed unit-variance etas + an sd theta", {
  skip_on_cran()
  skip_if_not(.stanHasIovSens(), "IOV gated off: upstream sens column wrong (nlmixr2est#952)")
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.iovMod, .iovData(), est = "stan", control = stanControl(run = FALSE))
  )
  ...
  # an opaque "_b2"/"_b3")
  expect_true(any(grepl("matrix[1,1] L_rx_iov_cl_1 = [[1]];", .lines, fixed = TRUE)))
  expect_true(any(grepl("matrix[1,1] L_rx_iov_cl_2 = [[1]];", .lines, fixed = TRUE)))
  expect_true(any(grepl("to_vector(z_rx_iov_cl_1) ~ std_normal();", .lines, fixed = TRUE)))
...
test_that("the assembled IOV target's gradient FD-agrees end to end", {
  ...
  .d <- .iovData()
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.iovMod, .d, est = "stan", control = stanControl(run = FALSE))
  )
  ...
  .Call(`_nlmixr2bayes_setMuRef`, as.integer(.code$map$muRefIdx))
  .sf <- rstan::sampling(
    .sm,
    data = .code$data,
    chains = 1,
...
test_that("plain fixed-variance etas work; fixed correlated blocks refuse", {
  ...
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.fixEta, .d, est = "stan", control = stanControl(run = FALSE))
  )
  .lines <- strsplit(.code$code, "\n")[[1]]
  expect_true(any(grepl("matrix[1,1] L_eta_cl = [[0.2]];", .lines, fixed = TRUE)))
  ...
  .codeC <- suppressMessages(
    nlmixr2est::nlmixr2(.fixEta, .d, est = "stan", control = stanControl(run = FALSE, etaParam = "centered"))
  )
  .linesC <- strsplit(.codeC$code, "\n")[[1]]
  expect_true(any(grepl("matrix[1,1] L_eta_cl = [[0.2]];", .linesC, fixed = TRUE)))
  expect_true(any(grepl("etaP_eta_cl[i] ~ multi_normal_cholesky", .linesC, fixed = TRUE)))
  if (requireNamespace("rstan", quietly = TRUE)) {
    expect_silent(rstan::stanc(model_code = .codeC$code, allow_undefined = TRUE))
  }
  ...
  expect_error(
    suppressMessages(
      nlmixr2est::nlmixr2(.fixBlk, .d, est = "stan", control = stanControl(run = FALSE))
    ),
    "correlations"
  )
})
...
test_that("a declared prior(iov.x) lands on the IOV magnitude theta", {
  ...
  skip_if_not(.stanHasIovSens(), "IOV gated off: upstream sens column wrong (nlmixr2est#952)")
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.iovPriorMod, .iovData(), est = "stan", control = stanControl(run = FALSE))
  )
  .lines <- strsplit(.code$code, "\n")[[1]]
...
test_that("an undeclared IOV magnitude gets the omega-SD default, not a leak", {
  ...
  .code2 <- suppressMessages(
    nlmixr2est::nlmixr2(
      ...
      .iovData(),
      est = "stan",
      control = stanControl(run = FALSE, diagOmegaSdPrior = "normal(0, %s)")
    )
  )
...
test_that("a matrix prior on an IOV magnitude is refused, not mis-emitted", {
  ...
  expect_error(
    suppressMessages(
      nlmixr2est::nlmixr2(.mvIov, .iovData(), est = "stan", control = stanControl(run = FALSE))
    ),
...
test_that("prior(iov.x) alone satisfies the est=\"stan\" prior requirement", {
  ...
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.onlyIov, .iovData(), est = "stan", control = stanControl(run = FALSE))
  )
  expect_true(any(grepl("iov_cl ~ cauchy(0, 1);", strsplit(.code$code, "\n")[[1]], fixed = TRUE)))
})
...
test_that("the capture hook is registered ahead of nlmixr2est's IOV rewrite", {
  ...
  expect_true(which(.hooks == ".stanCaptureIovPriors") < which(.hooks == ".uiApplyIov"))
})
...
test_that("several IOV parameters over several occasion variables", {
  ...
  .d$VIS <- .d$OCC
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.twoIov, .d, est = "stan", control = stanControl(run = FALSE))
  )
  .lines <- strsplit(.code$code, "\n")[[1]]
...
test_that("a parameter-dependent IOV prior is truncated at the SD's zero", {
  ...
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.depIov, .iovData(), est = "stan", control = stanControl(run = FALSE))
  )
  .lines <- strsplit(.code$code, "\n")[[1]]
  expect_true(any(grepl("iov_cl ~ cauchy(0, tauScale) T[0, ];", .lines, fixed = TRUE)))
  if (requireNamespace("rstan", quietly = TRUE)) {
    expect_silent(rstan::stanc(model_code = .code$code, allow_undefined = TRUE))
  }
})
