...
.refStanCode <- function() {
  paste(
    ...
    paste0("  sd_eta_cl ~ cauchy(0, ", format(2.5 * sqrt(0.1), digits = 15, trim = TRUE, scientific = FALSE), ");"),
...
test_that("G4: the linked posterior matches a hand-written pure-Stan reference", {
  ...
  .dataR <- list(N = 4L, Nobs = nrow(d), id = as.integer(d$ID), time = d$TIME, dv = d$DV)
  .sfR <- rstan::sampling(
    .smR,
    data = .dataR,
    chains = 2,
  ...
  for (.p in .pars) {
    ...
    expect_lt(abs(.sL[.p, "sd"] - .sR[.p, "sd"]) / .sR[.p, "sd"], 0.2, label = paste0("posterior sd of ", .p))
  }
})
