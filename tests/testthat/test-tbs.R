...
.tbsData <- function() {
  ...
  do.call(
    ...
    lapply(1:4, function(id) {
      rbind(
        data.frame(ID = id, TIME = 0, DV = NA_real_, AMT = 0, EVID = 2),
        data.frame(ID = id, TIME = .tt, DV = abs(5 * exp(-0.05 * .tt) + stats::rnorm(5, 0, 0.5)), AMT = 0, EVID = 0)
      )
    })
  )
}
...
test_that("estimated lambda: the linked d/dlambda column FD-agrees (#949)", {
  skip_on_cran()
  skip_if_not(nlmixr2bayes:::.stanHasTbsLambdaSens(), "nlmixr2est lacks the #949 lambda sensitivities")
...
test_that("estimated lambda: the generator emits the Jacobian statistic", {
  ...
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.tbsMod, .d, est = "stan", control = stanControl(run = FALSE))
  )
  expect_s3_class(.code, "nlmixr2bayesCode")
  .lines <- strsplit(.code$code, "\n")[[1]]
  expect_true(any(grepl("real sumLogJac_4;", .lines, fixed = TRUE)))
  expect_true(any(grepl("target += (lambda - 1) * sumLogJac_4;", .lines, fixed = TRUE)))
  # the statistic is exactly sum(log DV) for Box-Cox
  expect_equal(.code$data$sumLogJac_4, sum(log(.tbsObsDv(.d))), tolerance = 1e-12)
  ...
  .codeY <- suppressMessages(
    nlmixr2est::nlmixr2(.yjMod, .d, est = "stan", control = stanControl(run = FALSE))
  )
  .dvObs <- .tbsObsDv(.d)
  expect_equal(
    .codeY$data$sumLogJac_4,
  ...
  .codeF <- suppressMessages(
    nlmixr2est::nlmixr2(.fixMod, .d, est = "stan", control = stanControl(run = FALSE))
  )
  ...
  expect_error(
    suppressMessages(
      nlmixr2est::nlmixr2(.tbsMod, .dBad, est = "stan", control = stanControl(run = FALSE))
    ),
    "strictly positive"
  )
})
...
test_that("estimated lambda: the assembled target is the untransformed-scale density", {
  ...
  .sf <- rstan::sampling(
    ...
    cores = 1,
    show_messages = FALSE,
    init = list(list(tcl = 1, tv = 3, add_sd = 0.5, lambda = 0.5, sd_eta_cl = sqrt(0.1), z_eta_cl = matrix(0, 4, 1)))
  )
  .bt <- function(theta, e) {
    .Call(nlmixr2bayes:::`_nlmixr2bayes_condBatchTheta`, as.double(theta), as.matrix(e))
  }
  ...
  .pt <- function(lam) {
    list(tcl = 1, tv = 3, add_sd = 0.5, lambda = lam, sd_eta_cl = sqrt(0.1), z_eta_cl = .eta / sqrt(0.1))
  }
  .l1 <- 0.4
  .l2 <- 0.7
  .lp <- function(lam) {
...
test_that("estimated lambda on a logit base: statistic on the transformed DV; target exact", {
  ...
  .d <- do.call(
    ...
    lapply(1:4, function(id) {
      data.frame(
        ID = id,
        TIME = .tt,
        DV = pmin(19.5, pmax(0.5, 5 * exp(-0.05 * .tt) + stats::rnorm(5, 0, 0.5))),
        AMT = 0,
        EVID = 0
      )
    })
  ...
  .code <- suppressMessages(
    nlmixr2est::nlmixr2(.lgMod, .d, est = "stan", control = stanControl(run = FALSE))
  )
  # the statistic is the YJ statistic on the LOGIT-transformed DV
  .t <- stats::qlogis(.d$DV / 20)
  expect_equal(.code$data$sumLogJac_4, sum(log1p(.t[.t >= 0])) - sum(log1p(-.t[.t < 0])), tolerance = 1e-12)
