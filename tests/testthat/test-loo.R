...
")
  ...
  s <- rxode2::rxSolve(m, params = pm, events = dat, returnType = "data.frame", cores = 1L)
...
test_that("the generated program always emits log_lik", {
  ...
  expect_equal(
    length(gregexpr("normal_lpdf(dv[i] | pred[i], add_sd)", gen$code, fixed = TRUE)[[1]]),
    2L
  )
})
...
test_that("optimizing() and ADVI run through the bridge", {
  ...
  o <- rstan::optimizing(sm, data = gen$standata, seed = 5, init = rxsInit(gen, jitter = 0)(1L), verbose = FALSE)
